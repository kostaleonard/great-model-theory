...
trait DifferentiableFunction[T] {
  ...
  def getOutputShape(implicit classTag: ClassTag[T]): Array[Int] = computeOnZeroInputs match {
    case Success(outputs) => outputs.shape
  }

  private def computeOnZeroInputs(implicit classTag: ClassTag[T]): Try[NDArray[T]] = {
    val inputs = getInputs
    val zeroInputs = inputs.map(input => input -> NDArray.zeros[T](input.shape)).toMap
    compute(zeroInputs)
  }
}
...
case class Input[T: ClassTag](override val name: String, shape: Seq[Int]) extends Variable[T] {
...
case class MatMul[T](a: DifferentiableFunction[T], b: DifferentiableFunction[T])(implicit num: Numeric[T]) extends DifferentiableFunction[T] {
  override def compute(inputs: Map[Input[T], NDArray[T]]): Try[NDArray[T]] =
    a.compute(inputs) match {
      case Success(aValue) => b.compute(inputs) match {
